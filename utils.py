...
import requests
import json
# FIX: Ensure Any is included in the typing import
...
# --- Mathematical Models ---
# ... (Keep all mathematical model functions exactly as they were) ...
...
def black_scholes_merton_split_rates(S: float, K: float, T: float, r_drift: float, r_discount: float, sigma: float, option_type: str = 'call') -> float:
    ...
    d1 = (log_S_K + (r_drift + 0.5 * sigma**2) * T) / sigma_sqrt_T
    d2 = d1 - sigma_sqrt_T
    try:
...
def calculate_implied_volatility(target_price: float, S: float, K: float, T: float, r: float, option_type: str, low_bound: float = IV_LOW_BOUND, high_bound: float = IV_HIGH_BOUND, tol: float = IV_TOL, max_iter: int = IV_MAX_ITER) -> float:
    ...
    def objective(sigma: float) -> float:
        ...
        try:
            bsm_args = {'S': S, 'K': K, 'T': T, 'r': r, 'sigma': sigma, 'option_type': option_type}
            price = black_scholes_merton(**bsm_args)
            return price - target_price if pd.notna(price) else 1e12
...
def calculate_greeks(S: float, K: float, T: float, r: float, sigma: float, tau: float, option_type: str, model_func: Callable, model_args: Optional[Dict] = None, h_S_mult: float = 0.005, h_T_div: float = 365.25, h_sigma_abs: float = 0.001, h_r_abs: float = 0.0001) -> Dict[str, float]:
    ...
    h_S = max(S * h_S_mult, 1e-4); h_T = max(1.0 / h_T_div, MIN_TIME_LEVEL)
    h_sig = max(h_sigma_abs, 1e-5); h_r = max(h_r_abs, 1e-6)
    ...
    p0 = get_price(); pS_p = get_price(S=S + h_S); pS_m = get_price(S=S - h_S)
    pSig_p = get_price(sigma=sigma + h_sig); pSig_m = get_price(sigma=sigma - h_sig)
    pT_m = get_price(T=T - h_T)
    pR_p = get_price(r=r + h_r); pR_m = get_price(r=r - h_r)
    ...
    if pd.notna(pS_p) and pd.notna(pS_m) and pd.notna(p0):
        denom_g = h_S ** 2;
...
# --- Data Fetching ---
# ... (Keep all data fetching functions exactly as they were) ...
...
def get_stock_price(ticker_symbol: str) -> Optional[float]:
    ...
    try:
        ...
        price = info.get('last_price') or info.get('market_price') or info.get('regularMarketPrice')
        if price is None:
            full_info = ticker.info
            price = full_info.get('currentPrice') or full_info.get('regularMarketPrice') or full_info.get('bid') or full_info.get('ask') or full_info.get('previousClose')
...
def get_option_chain_data(_ticker_symbol: str, expiry_date: str) -> Optional[Dict[str, pd.DataFrame]]:
    ...
    try:
        ...
        if chain and hasattr(chain, 'calls') and hasattr(chain, 'puts'):
            proc_chain = {}
            cols_num = ['strike', 'lastPrice', 'bid', 'ask', 'volume', 'openInterest', 'impliedVolatility']
            ...
            for typ in ['calls', 'puts']:
                ...
                if df_orig is not None and not df_orig.empty:
                    df = df_orig.copy()
                    existing_cols_keep = [col for col in cols_keep if col in df.columns]
                    df = df[existing_cols_keep]
                    for col in cols_num:
                        if col in df.columns: df[col] = pd.to_numeric(df[col], errors='coerce')
                    df['mid'] = np.nan
                    if 'bid' in df.columns and 'ask' in df.columns:
                        valid_bid_ask = (df['bid'].notna() & df['ask'].notna() & (df['ask'] >= df['bid']) & (df['bid'] >= 0))
                        df.loc[valid_bid_ask, 'mid'] = (df.loc[valid_bid_ask, 'bid'] + df.loc[valid_bid_ask, 'ask']) / 2.0
                    if 'lastPrice' in df.columns:
                        valid_last_price = df['lastPrice'].notna() & (df['lastPrice'] >= 0)
                        df['mid'] = df['mid'].fillna(df.loc[valid_last_price, 'lastPrice'])
                    df['mid'] = pd.to_numeric(df['mid'], errors='coerce')
                    df['mid'] = df['mid'].apply(lambda x: x if pd.notna(x) and x >= 0 else np.nan)
                    if 'impliedVolatility' in df.columns:
                       df['impliedVolatility'] = df['impliedVolatility'].apply(lambda x: x if pd.notna(x) and IV_LOW_BOUND <= x <= IV_HIGH_BOUND else np.nan)
                    proc_chain[typ] = df.dropna(subset=['strike'])
                else: proc_chain[typ] = pd.DataFrame(columns=cols_keep + ['mid'])
            if not proc_chain.get('calls', pd.DataFrame()).empty or not proc_chain.get('puts', pd.DataFrame()).empty:
                return proc_chain
        return None
    except Exception: return None
...
def fit_garch_and_forecast(ticker_symbol: str, hist_start_date: date, hist_end_date: date, min_hist_points: int) -> Tuple[Optional[float], str]:
    if not ticker_symbol: return None, "No ticker symbol provided for GARCH."
    ...
    try: # Calculate simple vol fallback
        if hist is not None and not hist.empty and len(hist) > 1:
            log_ret_sv = np.log(hist['Close'] / hist['Close'].shift(1)).dropna()
            if not log_ret_sv.empty and log_ret_sv.std() > 1e-9:
                simple_vol = log_ret_sv.std() * np.sqrt(252)
                simple_vol = np.clip(simple_vol, MIN_VOL_LEVEL, MAX_VOL_LEVEL)
    except Exception: pass
    if hist is None or hist.empty:
        status = "No historical data for GARCH."; fallback_msg = f" Simple vol: {simple_vol:.4f}" if simple_vol is not None else " No fallback vol."
        return simple_vol, status + fallback_msg
    hist = hist.dropna(subset=['Close'])
    if len(hist) < min_hist_points:
        status = f"Insufficient data ({len(hist)} < {min_hist_points}) for GARCH."; fallback_msg = f" Simple vol: {simple_vol:.4f}" if simple_vol is not None else " No fallback vol."
        return simple_vol, status + fallback_msg
    try: log_ret = 100 * np.log(hist['Close'] / hist['Close'].shift(1)).dropna()
    except Exception as e:
        status = f"Log return error: {e}."; fallback_msg = f" Simple vol: {simple_vol:.4f}" if simple_vol is not None else " No fallback vol."
        return simple_vol, status + fallback_msg
    if log_ret.empty or log_ret.isnull().all():
        status = "No valid log returns."; fallback_msg = f" Simple vol: {simple_vol:.4f}" if simple_vol is not None else " No fallback vol."
        return simple_vol, status + fallback_msg
    if log_ret.std() < 1e-8:
        status = "Log returns std dev near zero."; fallback_msg = f" Simple vol: {simple_vol:.4f}" if simple_vol is not None else " No fallback vol."
        return simple_vol, status + fallback_msg
    try: # Fit GARCH
        model = arch_model(log_ret, vol='Garch', p=1, q=1, rescale=False)
        ...
        if res.convergence_flag != 0:
            status = f"GARCH converge failed (flag={res.convergence_flag})."; fallback_msg = f" Simple vol: {simple_vol:.4f}" if simple_vol is not None else " No fallback vol."
            return simple_vol, status + fallback_msg
        forecast = res.forecast(horizon=1, reindex=False)
        next_day_variance_scaled = forecast.variance.iloc[-1, 0]
        min_scaled_var = (MIN_VOL_LEVEL * 100)**2
        status_prefix = "GARCH OK. "
        if next_day_variance_scaled < min_scaled_var:
            last_cond_variance_scaled = res.conditional_volatility.iloc[-1]**2
            if last_cond_variance_scaled < min_scaled_var:
                status = "GARCH forecast & last var too low."; fallback_msg = f" Simple vol: {simple_vol:.4f}" if simple_vol is not None else " No fallback vol."
                return simple_vol, status + fallback_msg
        ...
        status = f"GARCH Model Error: {type(e).__name__}."; fallback_msg = f" Simple vol: {simple_vol:.4f}" if simple_vol is not None else " No fallback vol."
        return simple_vol, status + fallback_msg

# --- DoltHub API Functions ---
# ... (Keep DoltHub API functions exactly as they were) ...
@st.cache_data(ttl=CACHE_TTL)
def fetch_historical_options_via_api(ticker: str, start_date: Union[str, date], end_date: Union[str, date],
                                     strike: Optional[float] = None,
                                     expiry: Optional[Union[str, date]] = None,
                                     option_type: Optional[str] = None) -> pd.DataFrame:
    if not DOLTHUB_API_KEY: st.warning("DoltHub API Key not configured."); return pd.DataFrame()
    if not ticker: st.warning("Ticker symbol required for DoltHub query."); return pd.DataFrame()
    table_name = "options"; select_cols = "quote_date, expiration, strike, type, open, high, low, close, volume, implied_volatility, delta, gamma, theta, vega, rho, open_interest, bid, ask"; ticker_col = "underlying_symbol"
    where_clauses = [f"{ticker_col} = '{ticker.replace("'", "''")}'"]
    try:
        start_s = pd.to_datetime(start_date).strftime('%Y-%m-%d'); end_s = pd.to_datetime(end_date).strftime('%Y-%m-%d')
        where_clauses.append(f"quote_date BETWEEN '{start_s}' AND '{end_s}'")
    except ValueError: st.warning("Invalid start/end date."); return pd.DataFrame()
    ...
    if option_type is not None and option_type in ['call', 'put']: where_clauses.append(f"type = '{option_type}'")
    sql_query = f"SELECT {select_cols} FROM {table_name} WHERE {' AND '.join(where_clauses)} ORDER BY quote_date ASC"
    api_url = f"{DOLTHUB_API_BASE_URL}/{DOLTHUB_BRANCH}"; headers = {'Authorization': DOLTHUB_API_KEY, 'Accept': 'application/json'}
    params_api = {'q': sql_query}; df = pd.DataFrame()
    try:
        ...
        num_cols = ['strike', 'open', 'high', 'low', 'close', 'volume', 'implied_volatility', 'delta', 'gamma', 'theta', 'vega', 'rho', 'open_interest', 'bid', 'ask']
        for col in num_cols:
            if col in df.columns: df[col] = pd.to_numeric(df[col], errors='coerce')
        ...
        if 'bid' in df.columns and 'ask' in df.columns:
             ...
             if 'close' in df.columns: df['mid'] = df['mid'].fillna(df['close'])
             df['mid'] = df['mid'].apply(lambda x: x if pd.notna(x) and x >= 0 else np.nan)
        elif 'close' in df.columns: df['mid'] = df['close'].apply(lambda x: x if pd.notna(x) and x >= 0 else np.nan)
        df = df.set_index('quote_date').sort_index()
        df['TTM'] = (df['expiration'] - df.index).dt.days / 365.25
        df['TTM'] = df['TTM'].apply(lambda x: max(x, 0.0))
        df['TTM'] = df['TTM'].apply(lambda x: max(x, MIN_TIME_LEVEL) if x > 0 else 0.0)
        if 'implied_volatility' in df.columns:
             df['implied_volatility'] = df['implied_volatility'].apply(lambda x: x if pd.notna(x) and IV_LOW_BOUND <= x <= IV_HIGH_BOUND else np.nan)
        return df
...
def fetch_recent_distinct_options_api(ticker: str, limit: int = 50) -> pd.DataFrame:
    ...
    table_name = "options"; ticker_col = "underlying_symbol"; expiry_col = "expiration"
    strike_col = "strike"; type_col = "type"; date_col = "quote_date"
    ...
    try:
        ...
        df = df.dropna()
        df = df.sort_values(by=[expiry_col, strike_col, type_col]).reset_index(drop=True)
        return df
    except Exception: return df

# --- News Fetching & Sentiment ---
# ... (Keep news/sentiment functions exactly as they were) ...
def clean_text(text: Any) -> str:
    if not isinstance(text, str): return ""
    text = str(text); text = re.sub(r'http\S+|www\.\S+', ' ', text); text = re.sub(r'<.*?>', ' ', text)
    text = re.sub(r'\[.*?\]', ' ', text); text = re.sub(r'[\n\r\t]', ' ', text); text = re.sub(r'[^A-Za-z0-9\s.,!?$%]', '', text)
    text = re.sub(r'\s+', ' ', text).strip(); return text.lower()

@st.cache_data(ttl=1800)
def fetch_news_and_sentiment(newsapi_client: Optional[NewsApiClient], analyzer: SentimentIntensityAnalyzer, keywords: List[str], lang: str = 'en', page_size: int = 20) -> Tuple[pd.DataFrame, str]:
    if not newsapi_client: return pd.DataFrame(), "NewsAPI Client not initialized."
    if not keywords: return pd.DataFrame(), "No keywords provided."
    q_parts = [f'"{k}"' if ' ' in k else k for k in keywords if k and isinstance(k, str) and re.match(r'^[A-Za-z0-9\s"-]+$', k)]
    ...
    try:
        ...
        articles_response = newsapi_client.get_everything(q=query, language=lang, sort_by=sort_by, page_size=min(page_size, 100), from_param=from_date)
        if articles_response['status'] == 'ok':
            ...
            for article in articles:
                ...
                seen_urls.add(url); title = article.get('title', '') or "[No Title]"
                desc = article.get('description', '') or ""; content = article.get('content', '') or ""
            ...
            status_msg = f"Fetched and analyzed {len(news_df)} articles for: '{query}'."
            return news_df.sort_values('Pub', ascending=False), status_msg
        else:
            err_code = articles_response.get('code', '?'); err_msg = articles_response.get('message', 'Unknown error')
            status_msg = f"NewsAPI Error ({err_code}): {err_msg}"; st.error(status_msg); return pd.DataFrame(), status_msg
    except Exception as e:
        status_msg = f"News Fetch/Sentiment Error: {type(e).__name__} - {e}"; st.error(status_msg); return pd.DataFrame(), status_msg

# --- Greeks Visualization ---
# ... (Keep Greeks functions exactly as they were) ...
def create_gauge_chart(value: Optional[float], title: str, range_min: float, range_max: float, greek_symbol: str = "") -> go.Figure:
    fig = go.Figure()
    display_value = value if pd.notna(value) else 0; value_text = f"{value:.4f}" if pd.notna(value) else "N/A"
    ...
    fig.update_layout(height=220, margin=dict(l=20, r=20, t=40, b=10), font=dict(size=12))
    return fig
...
def display_greeks_gauges(greeks: Dict[str, float], option_type: str):
    ...
    for i, key in enumerate(greek_info):
        ...
        with cols[i]:
            st.plotly_chart(create_gauge_chart(val, info['name'], range_min, range_max, info['symbol']), use_container_width=True, config={'displayModeBar': False})
            st.caption(f"*{info['tip']}*", unsafe_allow_html=True)
